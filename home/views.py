...
def get_entire_taxipool_with_user(request, cursor, with_user=False):
    ...
    time_now = datetime.datetime.now()
    cursor.execute("UPDATE taxipool_seek_taxi SET condition = 'SSUCC' WHERE %s > depart_date + depart_time AND now_person >= num_person",[time_now])
    cursor.execute("UPDATE taxipool_seek_taxi SET condition = 'SFAIL' WHERE %s > depart_date + depart_time  AND now_person < num_person",[time_now])
